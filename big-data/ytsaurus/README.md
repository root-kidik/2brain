...
- [cli.md](cli.md) — как пользоваться `yt` CLI: установка, настройка подключения к кластеру (proxy/token), и все рабочие примеры команд (static/dynamic таблицы, операции, очереди, CHYT/YQL) в одном месте.
