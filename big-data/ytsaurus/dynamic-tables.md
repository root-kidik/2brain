...
- **Atomicity (атомарность транзакции)** — гарантия, что изменения транзакции наблюдаются все сразу или не наблюдаются вовсе: невозможно прочитать часть изменённых строк без остальных. По умолчанию (`full`) транзакции полностью атомарны через двухфазный commit, в т.ч. распределённые — одной транзакцией можно атомарно изменить строки в нескольких tablets и нескольких разных dynamic table сразу. Уровень можно понизить до `none` там, где эта гарантия не нужна, в обмен на меньшую latency записи.
