...
- CDC: коннектор читает changelog/WAL БД (а не делает periodic-опрос таблиц), поэтому видит каждое изменение строки, включая промежуточные, и публикует его в топик сразу после commit транзакции в БД — с минимальной задержкой и без дополнительной нагрузки на БД от polling-запросов.
- Event Sourcing: каждое доменное событие пишется в топик с ключом = идентификатор агрегата (например `order_id`), что гарантирует порядок событий одного агрегата внутри партиции; текущее состояние агрегата строится сверткой (fold) всех его событий по порядку, либо читается из периодически сохраняемого snapshot + событий после него (чтобы не проигрывать всю историю с нуля).
